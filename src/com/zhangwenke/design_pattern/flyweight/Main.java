 ...
 * 享元模式的主要有以下角色：
 *  -抽象享元角色（Flyweight）：通常是一个接口或抽象类，在抽象享元类中声明了具体享元类公共的方法，这些方法可以向
 *   外界提供享元对象的内部数据（内部状态），同时也可以通过这些方法来设置外部数据（外部状态）。
 *  -具体享元（Concrete Flyweight）角色 ：它实现了抽象享元类，称为享元对象；在具体享元类中为内部状态提供了
 *   存储空间。通常我们可以结合单例模式来设计具体享元类，为每一个具体享元类提供唯一的享元对象。
 *  -非享元（Unsharable Flyweight)角色 ：并不是所有的抽象享元类的子类都需要被共享，不能被共享的
 ...
 *  -简单的说有一个对象被大量使用，并且该对象里面的属性是一样并且不会被修改的，可以使用
 */
public class Main {
    public static void main(String[] args) {
        ...
        ticket1.show("商务座");
        ITicket ticket2 = TicketFactory.getTicket("上海", "深圳");
        ticket2.show("二等座");
        System.out.println(ticket == ticket1);//true
        System.out.println(ticket == ticket2);//true
